...
static void repeatStatement(void) {
  ...
  expression();
  consume(TOKEN_RIGHT_PAREN, "Expect ')' after repeat count.");

  beginScope();

  static const char repeatCounterName[] = "__repeatCounter";
  Token counterToken;
  counterToken.type = TOKEN_IDENTIFIER;
  counterToken.start = repeatCounterName;
  counterToken.length = (int)(sizeof(repeatCounterName) - 1);
  counterToken.line = parser.previous.line;

  addLocal(counterToken, false);
  markInitialized();
  int counterSlot = current->localCount - 1;

  int conditionStart = currentChunk()->count;
  emitByte(OP_GET_LOCAL);
  emitShort((uint16_t)counterSlot);
  ...
  beginLoop(decrementStart);
  patchJump(bodyJump);
  statement();
  endLoop();

  emitLoop(decrementStart);
