  ...
  if (IS_OBJ(callee)) {
    switch (OBJ_TYPE(callee)) {
      ...
      case OBJ_NATIVE: {
        ...
        Value result;
        if (CLOX_NATIVE_SAFE_CALLS && !native->function(argCount, vm.stackTop - argCount, &result)) {
          return false;
        }
...
static InterpretResult run(void) {
  ...
  #define IP_OFFSET() ((int)(ip - frame->closure->function->chunk.code))
  #else
  ...
  #define IP_OFFSET() ((int)(frame->ip - frame->closure->function->chunk.code))
  #endif
...
#define BINARY_OP(valueType, op)                       \
  ...
  for (;;) {
    ...
    printf("\n");
    disassembleInstruction(&frame->closure->function->chunk, IP_OFFSET());
#endif
