...
class Parser {
  ...
  private Stmt classDeclaration() {
    ...
    while (!check(RIGHT_BRACE) && !isAtEnd()) {
      ...
      if (!check(LEFT_PAREN)) {
        ...
        methods.add(new Stmt.Function(methodName, new ArrayList<>(), body, isStatic, isGetter));
      } else {
        ...
        List<Stmt> body = block();
        methods.add(new Stmt.Function(methodName, parameters, body, isStatic, isGetter));
      }
    }
  ...
  private Stmt.Function function(String kind) {
    ...
    List<Stmt> body = block();
    return new Stmt.Function(name, parameters, body, false, false);
  }
  ...
  private Expr ternary() {
    ...
    if (match(QUESTION)) {
      ...
      Token colon = consume(COLON, "Expect ':' after then branch of ternary expression.");
      Expr elseBranch = ternary();
      return new Expr.Ternary(expr, question, thenBranch, colon, elseBranch);
    }
