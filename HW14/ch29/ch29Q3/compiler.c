...
static void inner_(bool canAssign) {
  (void)canAssign;
  if (currentClass == NULL || !currentClass->inMethod) {
  ...
  uint8_t name = identifierConstant(&currentClass->methodName);
  namedVariable(syntheticToken("this"), false);
...
static void method(void) {
  ...
  bool previousInMethod = currentClass->inMethod;
  Token previousMethodName = currentClass->methodName;
  currentClass->inMethod = true;
  currentClass->methodName = methodName;
  function(type);
  currentClass->inMethod = previousInMethod;
  currentClass->methodName = previousMethodName;
...
static void classDeclaration(void) {
  ...
  classCompiler.inMethod = false;
  classCompiler.methodName = syntheticToken("");
  ...
  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF)) {
    ...
    if (match(TOKEN_CASE)) {
      ...
      if (endJumpCount == endJumpCapacity) {
        ...
        endJumps = checkedRealloc(endJumps, sizeof(int) * (size_t)endJumpCapacity);
      }
