...
static void splitBlock(BlockHeader* block, size_t desiredSize) {
  ...
  BlockHeader* split = (BlockHeader*)((uint8_t*)headerToPayload(block) + alignedSize);
...
static void rcAddObjectRef(Obj** objects, int objectCount, int* refCounts, Obj* target) {
  if (target == NULL) return;
  int index = findObjectIndex(objects, objectCount, target);
  if (index >= 0) refCounts[index]++;
}

static void rcAddValueRef(Obj** objects, int objectCount, int* refCounts, Value value) {
  if (IS_OBJ(value)) rcAddObjectRef(objects, objectCount, refCounts, AS_OBJ(value));
}

static void rcAddTableRefs(Obj** objects, int objectCount, int* refCounts, Table* table) {
...
static void rcAddInternalRefs(Obj** objects, int objectCount, int* refCounts, Obj* object) {
  switch (object->type) {
    ...
    case OBJ_CLASS:
      rcAddObjectRef(objects, objectCount, refCounts, (Obj*)((ObjClass*)object)->name);
      rcAddObjectRef(objects, objectCount, refCounts, (Obj*)((ObjClass*)object)->superclass);
      rcAddTableRefs(objects, objectCount, refCounts, &((ObjClass*)object)->methods);
      rcAddTableRefs(objects, objectCount, refCounts, &((ObjClass*)object)->innerMethods);
      break;
    case OBJ_CLOSURE: {
      ObjClosure* closure = (ObjClosure*)object;
      rcAddObjectRef(objects, objectCount, refCounts, (Obj*)closure->function);
      rcAddObjectRef(objects, objectCount, refCounts, (Obj*)closure->ownerClass);
      rcAddObjectRef(objects, objectCount, refCounts, (Obj*)closure->methodName);
    ...
    case OBJ_FUNCTION: {
      ...
      for (int i = 0; i < function->chunk.constants.count; i++) {
        rcAddValueRef(objects, objectCount, refCounts, function->chunk.constants.values[i]);
      }
      break;
    }
    case OBJ_INSTANCE: {
...
static void collectAcyclicGarbage(void) {
  ...
  if (objects == NULL || refCounts == NULL || queued == NULL || reclaim == NULL || queue == NULL) {
    free(objects);
    free(refCounts);
    free(queued);
    free(reclaim);
    ...
    switch (object->type) {
      ...
      case OBJ_CLASS:
        if (((ObjClass*)object)->name != NULL) {
          int target = findObjectIndex(objects, objectCount, (Obj*)((ObjClass*)object)->name);
...
static void blackenObject(Obj* object) {
  ...
  switch (object->type) {
    ...
    case OBJ_CLASS: {
      ...
      markTable(&klass->innerMethods);
      break;
    }
    case OBJ_CLOSURE: {
      ObjClosure* closure = (ObjClosure*)object;
      markObject((Obj*)closure->function);
      markObject((Obj*)closure->ownerClass);
...
static void freeObject(Obj* object) {
  ...
  switch (object->type) {
    ...
    case OBJ_CLASS:
      ...
      freeTable(&((ObjClass*)object)->innerMethods);
