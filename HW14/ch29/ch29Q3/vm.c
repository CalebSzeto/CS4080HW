...
#ifndef CLOX_FAST_INIT_CACHE
#define CLOX_FAST_INIT_CACHE 1
#endif
...
static ObjClass* findInnerTargetClass(ObjClass* receiverClass, ObjClass* ownerClass, ObjString* name) {
  ObjClass* chain[256];
  int chainCount = 0;
...
static bool invokeInner(CallFrame* frame, ObjString* name) {
  ...
  ObjClass* target = findInnerTargetClass(instance->klass, frame->closure->ownerClass, name);
  if (target == NULL) {
    pop();
    push(NIL_VAL);
    return true;
...
#if CLOX_FAST_INIT_CACHE
        initializer = klass->initializer;
        if (!IS_NIL(initializer)) {
#else
        if (tableGet(&klass->methods, OBJ_VAL(vm.initString), &initializer)) {
#endif
          ObjClosure* closure = IS_CLOSURE(initializer)
              ? AS_CLOSURE(initializer)
              : newClosure(AS_FUNCTION(initializer));
          return call(closure, argCount);
        } else if (argCount != 0) {
...
static bool typeNative(int argCount, Value* args, Value* result) {
  ...
  if (IS_BOOL(args[0])) {
    ...
    switch (OBJ_TYPE(args[0])) {
      case OBJ_BOUND_METHOD: name = "function"; break;
